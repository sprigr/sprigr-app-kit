...
- `/oauth/start` puts the `installId` in the state (from `env.INSTALL_ID`, bound by #851), encodes with `encodeState()`, and uses the bouncer URL as `redirect_uri`. The bouncer URL is environment-aware:
  ...
  ```
- The bouncer dispatches to `/__sprigr/tool/<app-slug-snake-case>_oauth_callback`. Your manifest **must** declare that tool (`procore_oauth_callback` in our case) and point its `handler` at a module that exchanges the code with Procore. The full dispatch body is `{ code, state, redirectUri, environment, installId, providerParams }` — `state` is the raw encoded state from `/oauth/start`, so the handler can decode it and verify the `csrf` against the `oauth_csrf` value it stashed in D1; report a mismatch as `{ ok: false, error: 'expired_or_unknown_csrf' }` and the bouncer surfaces it as a real error page instead of a false success.
- `providerParams` carries every extra query param the provider appended to the callback redirect, minus `code`/`state` (already top-level). Always present; `{}` when the provider added nothing. This is the only route for an identifier that exists on the redirect and nowhere else: MYOB's `businessId` (the company file every API call is scoped to — unavailable from the API itself for keys issued after 2025-03-12), QuickBooks Online's `realmId`, or a provider echoing the granted `scope`. Persist what you need in the callback; it is not recoverable later. Because the callback tool's `input_schema` doesn't set `additionalProperties: false`, the field arrives even though your manifest doesn't declare it — declaring it is still worth doing as documentation.
...
- **Bouncer emulation (§6)**: `GET http://localhost:8666/<slug>/oauth/callback?code&state` decodes the state and dispatches `{ code, state, redirectUri, environment, installId }` to your `<slug>_oauth_callback` tool, with the real bouncer's success/failure rendering (including the inner `ok:false` check). **The local harness does not forward `providerParams`** — that path is the deployed bouncer only, so a handler branch that depends on `businessId`/`realmId` has to be unit-tested with a synthetic `providerParams` and then confirmed on staging. Register the localhost callback URL on your provider's **dev** OAuth app and set `<PREFIX>_REDIRECT_URI` to it; the whole consent → csrf-verify → token-exchange → D1-persist loop then runs on your machine. `GET /dev/state?csrf=...` mints a wire-correct state blob.
