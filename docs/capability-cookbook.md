...
- **[`examples/mock-warehouse`](../examples/mock-warehouse)** and **[`examples/mock-order-source`](../examples/mock-order-source)**: the two sides of the fulfilment hub's [interface contract](interfaces/fulfilment-hub-v1.md), a `fulfilment_provider` and an `order_source` implementer. Each claims every op of its interface with a `provides` tag and is deterministic enough to be a shakedown fixture, so they are the worked reference for an adapter that has to be interchangeable with another vendor's.
...
| Confirmation policy (T1) | `tools[].confirmation` (`always` / `when.count` / `actions`; `confirmation_required: true` is legacy sugar) | `showcase` tool | n/a (platform-enforced before dispatch; model-attested) | `xero-accounting` (generated from a policy file + coverage test), `shopify` |
| Human approval (T2) | handler returns `_approval` | `requireApproval` in `@sprigr/apps-app-sdk`; see [docs/write-protection.md](write-protection.md) | staging (needs a real tap) | `shopify` |
| Undo (T3) | `undo.reverse_tool` + handler returns `_undo` | `runUndoApply` + `@sprigr/apps-undo-journal`; see [docs/write-protection.md](write-protection.md) | staging (platform mints the token) | `shopify`, `xero-accounting` |
...
| Webhook: Google OIDC (GCP push) | `webhooks[]` (`signature.type: google_oidc`, optional `oidc_email` pin) | google-workspace `gws_handle_gmail_push` (sprigr-apps) | n/a (platform verifies vs Google JWKS; audience = the delivery URL) | `google-workspace` (Gmail Pub/Sub push) |
| Webhook: provider handshake (echo-the-secret) | `webhooks[]` (`signature.type: handshake_hmac`, `signature.handshake.header`, optional `handshake.key_query`) | asana `handle_asana_webhook` (sprigr-apps, in flight) | n/a (the platform answers the handshake, stores the provider-minted secret, and verifies later deliveries; see [platform-reference](platform-reference.md#provider-handshakes-signaturetype-handshake_hmac)) | `asana` (X-Hook-Secret / X-Hook-Signature) |
...
| Interface: CONFORMANCE harness | none (a test dependency) | [`packages/fulfilment-conformance`](../packages/fulfilment-conformance), used by [`mock-warehouse`](../examples/mock-warehouse/__tests__/conformance.test.ts) and [`mock-order-source`](../examples/mock-order-source/__tests__/conformance.test.ts) | **local** (pure logic + a fake env) | none yet (first users: the fulfilment hub adapters) |
| Interface: ADAPTER scaffold | `cross_tenant_tools[].provides` + `events.emits[]`, generated | `pnpm create:app <slug> --template fulfilment-provider\|order-source` ([`tools/templates/fulfilment-adapter.mjs`](../tools/templates/fulfilment-adapter.mjs)) | **local** (the generated app is green on `pnpm test`) | none yet |
