...
| [docs/canvas-builders-guide.md](docs/canvas-builders-guide.md) | **Building a tool that draws a company, not an app?** Step-by-step: read a company over MCP, keep it live over the company events socket, chat with agents, answer decision cards, start workflows, and push events both ways. Helpers ship as `@sprigr/canvas-kit`. Cold-built by an engineer with no Sprigr context. |
